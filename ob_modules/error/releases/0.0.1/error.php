...
if( ! function_exists('error_secure_path')) 
{
    function error_secure_path($file, $search_paths = FALSE)
    {
        if($search_paths)
        {
            $replace = array('APP'. DS, 'BASE'. DS, 'MODULES'. DS, 'ROOT'. DS, 'OB_MODULES'. DS);
            
            return str_replace(array(APP, BASE, MODULES, ROOT, OB_MODULES), $replace, $file);
        }
        ...
        elseif (strpos($file, OB_MODULES) === 0)
        {
            $file = 'OB_MODULES'. DS .substr($file, strlen(OB_MODULES));
        }
