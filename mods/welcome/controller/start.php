<?php

Class Start extends Controller {    
                                      
    function __construct()
    {   
        parent::__construct();
       
        new form\start();
        new form_Json\start();
        new sess\start();
        
        if(db('username') == '')
        {
            showError('SETUP ERROR - Please create a test database called "test" 
                and configure it from "/app/config/database.php", then run the "test.sql"</b> which is located in your /modules/tutorials/ folder');
        }
        
        log\me('debug', '[ welcome ]: Example log, who want to keep the logs in different color.');
    }         

    public function index()
    {    
        $data = array();
        $data['user'] = array();
        
        view('form', $data, false);
    }
    
    function ajax_example()
    {
        $data = array();
        
        view('view_form_ajax', $data, false);
    }
    
    function do_post()
    {   
        loader::model('user', false);  // Include user model
        
        $user = new User(new UserSchema);
        $user->user_password = i_get_post('user_password');
        $user->user_email    = i_get_post('user_email');
        
        $data = array();
        $data['user'] = $user;  // User object for none ajax request
  
        if($user->save())
        {
            if(i_ajax())  // Ajax request
            {
                echo form_json_success($user);
                return;
            }
            else    // Http request
            {
                sess_set_flash('notice', 'Form saved succesfully !');
                
                redirect('/welcome/start');
            }
        } 
        else
        {
            if(i_ajax()) // Ajax request
            {
                echo formJsonError($user);
                return;
            }
        }
        
        view('view_form', $data, false);    
    }
   
    public function delete()
    {
        loader::model('user', false);  // Include user model
        
        $user = new User();
        $user->where('usr_id', 1);
        
        if($user->delete())
        {
            echo 'User Deleted Successfuly !';
        } 
        
        print_r($user->errors());
    }
    
}

/* End of file start.php */
/* Location: .modules/test/controllers/start.php */